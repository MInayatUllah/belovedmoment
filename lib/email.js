...
export async function sendAdminNotificationEmail(orderDetails) {
  try {
    ...
    const processingHours = orderDetails.processing_time === '24h' ? 24 : 36;
...
export async function sendContactFormEmail(email, message) {
  try {
    const formData = new FormData();
    const apiKey = process.env.MAILEROO_API_KEY;
    ...
    formData.append('to', 'Support@belovedmoment.com');
    formData.append('subject', `New Contact Form Message from ${email}`);
    formData.append('html', `
      <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px;">
        ...
        <p style="font-size: 12px; color: #6b7280; margin-top: 20px;">This is an automated notification from the BelovedMoment contact form.</p>
      </div>
    `);
    ...
    if (!response.ok || !result.success) {
      throw new Error(result.message || 'Contact form email sending failed');
    }
